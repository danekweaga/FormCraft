...
const CATEGORIES: Category[] = [
  ...
  },
  {
    ...
    terms: [
      // Never match standalone "ai" — that admits fruit/ASMR generative slop.
      "artificial intelligence", "chatgpt", "openai", "claude", "gemini",
      "cursor", "codex", "copilot", "llm", "large language model", "ai agent",
      "agentic", "vibe coding", "prompting", "mcp", "machine learning",
      "hallucination", "ai tool", "ai workflow", "ai coding", "ai for coding",
      "ai programmer", "coding with ai", "model release", "new model",
      "open source model", "huggingface", "anthropic", "deepseek",
      "grok", "xai", "gpt", "foundation model",
    ],
  },
  {
...
const AI_GENERATION_MARKERS = [
  ...
  "made by ai", "made with veo", "made with sora", "made with kling",
  "google veo", "veo 3", "veo3", "sora 2", "sora ai", "runway gen",
  "kling ai", "hailuo", "grok imagine", "luma dream", "pika labs",
  "midjourney", "ai animation", "ai movie", "ai film", "ai short film",
  "ai art", "ai slop", "will smith spaghetti", "will smith eating",
];

const AI_SLOP_SUBJECTS = [
  "fruit", "watermelon", "strawberry", "mango", "kiwi", "dragon fruit",
  "asmr", "satisfying", "mukbang", "hyperrealistic", "candy", "dessert",
];

const EXPLICIT_AI_SLOP_PHRASES = [
  "ai fruit", "aifruit", "ai watermelon", "ai food", "ai asmr",
  "fruit asmr", "satisfying fruit", "satisfying ai", "cutting fruit",
  "ai generated fruit", "ai generated food", "ai generated asmr",
  "hyperrealistic ai", "ai cinematic", "ai trailer",
];

const AI_BRAND_MARKERS = [
  "ai", "gpt", "chatgpt", "grok", "veo", "sora", "midjourney",
  "runway", "kling", "pika", "hailuo", "luma",
];

const DEV_SLOP_RESCUE_TERMS = [
  "coding", "programmer", "programming", "software engineer",
  "software engineering", "cs student", "computer science", "hackathon",
  "leetcode", "debugging", "developer", "github", "internship",
...
export function isHardExclusionReason(reason: string): boolean {
  return HARD_EXCLUSION_REASON_PATTERN.test(reason);
}
...
function looksLikeAiEntertainmentSlop(
  haystack: string,
  rawMetadata: string,
): string[] {
  if (matchedTerms(haystack, DEV_SLOP_RESCUE_TERMS).length > 0) return [];

  const explicit = matchedTerms(haystack, EXPLICIT_AI_SLOP_PHRASES);
  if (explicit.length > 0) return explicit;

  const brands = matchedTerms(haystack, AI_BRAND_MARKERS);
  const subjects = matchedTerms(haystack, AI_SLOP_SUBJECTS);
  if (brands.length > 0 && subjects.length > 0) {
    return [...brands.slice(0, 2), ...subjects.slice(0, 3)];
  }

  const generation = matchedTerms(haystack, AI_GENERATION_MARKERS);
  const genRegex =
    /\b(veo\s*3|veo3|sora\s*2|runway\s*gen|kling\s*ai|grok\s*imagine|ai[\s-]*generated|aigenerated)\b/i;
...
export function isHardExcludedResearchItem(
  ...
  const result = classifyCreatorContentUniverse(item);
  return !result.relevant && isHardExclusionReason(result.reason);
}
...
export function classifyCreatorContentUniverse(
  ...
  const slopMatches = looksLikeAiEntertainmentSlop(detailHaystack, rawMetadata);
  if (slopMatches.length > 0) {
    return {
      relevant: false,
      category: null,
      reason: "Matches excluded AI entertainment slop",
      matchedTerms: slopMatches.slice(0, 5),
    };
  }
