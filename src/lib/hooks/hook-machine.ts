...
export async function generateHookPackFromResearch(params: {
  ...
  const fallback: HookPack = {
    formatMatched: sourceHook
      ? [
          {
            text: sourceHook.slice(0, 180),
            grade: "B",
            note: "Source pattern held as a format seed. Rewrite with your own proof before filming.",
            formatLabel: "Source pattern",
          },
        ]
      : [],
    original: [
  ...
  const result = await tryStructuredAI({
    ...
    input: {
      ...
      messages: [
        {
          role: "system",
          content: HOOK_MACHINE_SYSTEM_PROMPT,
        },
        {
          role: "user",
